...
def run_cli() -> None:
    ...
    # ── 13. Chunk-corner markers ──────────────────────────────────────────────
    console.print()
    use_chunk_corners = _ask_confirm(
        "Place a block at each chunk corner (every 16×16 blocks)?",
        default=False,
    )
    chunk_corner_block: str | None = None
    if use_chunk_corners:
        console.print(
            "[dim]A block is placed at every (col%16==0, row%16==0) position in the\n"
            "schematic footprint — i.e. wherever chunk boundary lines intersect.\n"
            "Common choices: [bold]Sea Lantern[/bold], [bold]Glowstone[/bold], [bold]Shroomlight[/bold]\n"
            "for light, or [bold]Crying Obsidian[/bold] for a map-visible purple marker.\n"
            "The marker overwrites whatever would otherwise be at that position.[/dim]\n"
        )
        chunk_corner_block = _ask_block("Chunk corner marker block:")

    # ── 14. Background / surface block ───────────────────────────────────────
    console.print()
    ...
    grid = render_text(
        ...
        padding=padding,
        chunk_corners=use_chunk_corners,
    )
    ...
    if use_chunk_corners and chunk_corner_block:
        block_table.add_row(
            "Chunk corners ◆",
            chunk_corner_block,
            _total(block_counts["chunk_corner"]),
        )
    block_table.add_row(
    ...
    with console.status("[bold cyan]Generating schematic…[/bold cyan]", spinner="dots"):
        saved_path = save_schematic(
            ...
            chunk_corner_block=chunk_corner_block if use_chunk_corners else None,
        )
